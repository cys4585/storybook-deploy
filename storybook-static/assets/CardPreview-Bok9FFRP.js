import{j as r}from"./jsx-runtime-BnIj46N_.js";import{c as h}from"./cardIssuers-BjBf63as.js";import{u as i}from"./styled-components.browser.esm-BDDUX98t.js";const g="data:image/png;base64,",l="data:image/png;base64,",c=e=>e[0].startsWith("4"),o=e=>["51","52","53","54","55"].filter(a=>e[0].startsWith(a)).length>0;function m({cardInformation:e,isFlipped:a}){const d=c(e.cardNumbers)?l:o(e.cardNumbers)?g:null;return r.jsx(u,{children:a?r.jsx(k,{children:r.jsx(R,{children:r.jsx(y,{children:e.cardCvc})})}):r.jsxs(v,{cardIssuer:e.cardIssuer,children:[r.jsxs(C,{children:[r.jsx(M,{}),d&&r.jsx(j,{src:d,alt:"Payment Company Logo"})]}),r.jsxs(b,{children:[r.jsxs(s,{children:[r.jsx(t,{children:e.cardNumbers[0]}),r.jsx(t,{children:e.cardNumbers[1]}),r.jsx(A,{children:Array.from({length:e.cardNumbers[2].length}).map((x,n)=>r.jsx(p,{},n))}),r.jsx(A,{children:Array.from({length:e.cardNumbers[3].length}).map((x,n)=>r.jsx(p,{},n))})]}),r.jsx(s,{children:r.jsxs("div",{children:[r.jsx(t,{children:e.cardExpiration.month}),e.cardExpiration.month.length===2&&"/",r.jsx(t,{children:e.cardExpiration.year})]})}),r.jsx(s,{children:r.jsx(t,{children:e.cardOwnerName})})]})]})})}const u=i.div`
  width: 212px;
  height: 132px;
  box-shadow: 3px 3px 5px 0px rgba(0, 0, 0, 0.25);
  border-radius: 4px;
`,v=i.div`
  width: 100%;
  height: 100%;
  background: ${e=>e.cardIssuer===""?"rgba(51, 51, 51, 1)":h[e.cardIssuer].color};
  border-radius: 4px;
  color: rgba(255, 255, 255, 1);
`,C=i.div`
  padding: 8px 12px 0;
  display: flex;
  justify-content: space-between;
`,M=i.div`
  width: 36px;
  height: 22px;
  background: rgba(221, 205, 120, 1);
  border: 0.5px solid rgba(221, 205, 120, 0.1);
  border-radius: 4px;
`,j=i.img`
  width: 36px;
  height: 22px;
`,b=i.div`
  display: flex;
  flex-direction: column;
  padding: 14px 25px 12px 17px;
  gap: 8px;
`,s=i.div`
  display: flex;
  justify-content: flex-start;
  align-items: center;
  min-height: 20px;
  gap: 10px;
`,t=i.span`
  font-family: Inter;
  font-size: 14px;
  font-weight: 500;
  line-height: 20px;
  letter-spacing: 0.08em;
  text-align: left;
  min-width: 38px;
`,A=i.div`
  display: flex;
  justify-content: center;
  align-items: center;
  gap: 5px;
`,p=i.div`
  width: 4px;
  height: 4px;
  border-radius: 50%;
  background: rgba(255, 255, 255, 1);
`,k=i.div`
  width: 100%;
  height: 100%;
  background: rgba(213, 213, 213, 1);
  display: flex;
  align-items: end;
  border-radius: 4px;
  color: rgba(255, 255, 255, 1);
`,R=i.div`
  margin-bottom: 24px;
  width: 100%;
  height: 24px;
  background: rgba(203, 186, 100, 1);
  display: flex;
  justify-content: flex-end;
`,y=i.span`
  margin-right: 16px;
`;m.__docgenInfo={description:"",methods:[],displayName:"CardPreview",props:{cardInformation:{required:!0,tsType:{name:"CardInformationValueState"},description:""},isFlipped:{required:!0,tsType:{name:"boolean"},description:""}}};export{m as C};
